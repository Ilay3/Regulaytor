...
INT CFlatHeaderCtrl::DrawText(CDC* pDC, CRect rect, LPHDITEM lphdi)
{
	...
	if(rect.Width()>0 && lphdi->mask&HDI_TEXT && lphdi->fmt&HDF_STRING)
	{
		...
		UINT MyMASK=lphdi->fmt&RVCF_JUSTIFYMASK;
		if(MyMASK & RVCF_LEFT)
		{
			flags|=DT_LEFT;
		}
		...
		{
			flags|=DT_SINGLELINE;	//если НЕ зашли сюда - можно ставить крест на вертикальном выравнивании
		}
		...
		pDC->DrawText(lphdi->pszText, -1, rect, DT_END_ELLIPSIS | flags);
	}
...
BOOL CFlatHeaderCtrl::OnToolTipNotify(UINT nId, NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if(
		...
		wcscpy((WCHAR*)pTTT->lpszText, T2W((LPTSTR)m_arrayHdrItemEx[m_iHotIndex].strToolTip.GetBuffer()));
