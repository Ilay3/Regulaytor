...
//			- Fixed FindItem for LPARAM as suggested by Armin Zьrcher.
...
BOOL CReportData::GetSubItem(INT iSubItem, LPINT lpiImage, LPINT lpiOverlay, LPINT lpiCheck, LPINT lpiColor, LPTSTR lpszText, LPINT lpiTextMax)
{
	...
	LPCTSTR lpsz = /*m_pchData*/GetBuffer();
	lpsz = &lpsz[iPos];
...
/*****************************	DrawText(CDC* pDC, CRect rect, LPRVITEM lprvi)	********************
*	Ёта функци€ отрисовывает текст €чеек (не заголовков). јвтоматически настраиваетс€ высота
*	текста с учетом переноса строки.
***************************************************************************************************/
INT CReportCtrl::DrawText(CDC* pDC, CRect rect, LPRVITEM lprvi)
{
	...
	if(rect.Width()>0 && lprvi->nMask&RVIM_TEXT)
	{
		...
		CRect rect2=rect;	//—копируем предыдущий рект
			//и расширим его вниз до состо€ни€, которое требуетс€ дл€ показа всего
			//текста
		rect2.bottom=rect2.top+this->PreviewHeight(NULL,lprvi->lpszText,NULL);
 		UINT flags=0;	//флаги, с которыми нужно отрисовывать текст
		UINT MyMASK=m_arraySubItems[lprvi->iSubItem].nFormat&RVCF_JUSTIFYMASK;
		if(MyMASK & RVCF_LEFT)
		{//777case HDF_LEFT|HDF_RTLREADING:
			flags|=DT_LEFT;
			//pDC->DrawText(lprvi->lpszText, -1, rect2, DT_LEFT|DT_END_ELLIPSIS/*|DT_SINGLELINE*/|DT_VCENTER);
		}
		if(MyMASK & RVCF_CENTER)
		{
			flags|=DT_CENTER;
		//777case HDF_CENTER|HDF_RTLREADING:
			//pDC->DrawText(lprvi->lpszText, -1, rect2, DT_CENTER|DT_END_ELLIPSIS/*|DT_SINGLELINE*/|DT_VCENTER);
		}
		if(MyMASK & RVCF_RIGHT)
		{//777case HDF_RIGHT|HDF_RTLREADING:
			flags|=DT_RIGHT;
			//pDC->DrawText(lprvi->lpszText, -1, rect2, DT_RIGHT|DT_END_ELLIPSIS/*|DT_SINGLELINE*/|DT_VCENTER);
		}
		if(MyMASK & RVCF_SINGLELINE)
		{
			flags|=DT_SINGLELINE;	//если Ќ≈ зашли сюда - можно ставить крест на вертикальном выравнивании
		}
		if(MyMASK & RVCF_VCENTER)
		{//нужно раст€нуть область рисовани€ на всю €чейку
			rect2.bottom=rect2.top+GetItemHeight()/*+lprvi->nPreview+10*/;
			flags|=DT_VCENTER;		//вертикальное выравнивание по середине
		}
		//собственно прорисовка текста
		pDC->DrawText(lprvi->lpszText, -1, rect2, DT_END_ELLIPSIS | flags);
	}
...
BOOL CReportCtrl::BeginEdit(INT iRow, INT iColumn, UINT nKey)
{
	...
	if(!bResult)
	{
		...
		//pWnd->BeginEdit(nKey);	//это убрал €
		//а это дл€ того, чтобы в начале изменени€ выдел€лс€ весь текст €чейки
		pWnd->SetSel(0,_tcsnlen(szText,REPORTCTRL_MAX_TEXT));
...
void CReportCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if(!Notify(&nmrv))
	{
		...
		case VK_ESCAPE:
			break;
		/*case VK_RETURN:
			BeginEdit(m_iFocusRow,m_iIndentColumn,0);
			break;*/
