...
BEGIN_MESSAGE_MAP(CFlagsDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_LOGTIME, OnBnClickedLogtime)
	ON_BN_CLICKED(IDC_LOG_BS_IN, &CFlagsDlg::OnBnClickedLogBsIn)
	ON_WM_CTLCOLOR()
...
BOOL CFlagsDlg::OnInitDialog()
{
	...
	o_LogTimeMessages1=pUSART_Dlg->pPROTOCOL->pProtocolOptions->o_LogTimeMessages;
	((CButton*)GetDlgItem(IDC_LOGTIME))->SetCheck(o_LogTimeMessages1);

	o_LogInByteStuffing1 = pUSART_Dlg->pPROTOCOL->pProtocolOptions->o_LogInByteStuffing;
	((CButton*)GetDlgItem(IDC_LOG_BS_IN))->SetCheck(o_LogInByteStuffing1);
...
void CFlagsDlg::OnBnClickedLogBsIn()
{
	if (((CButton*)GetDlgItem(IDC_LOG_BS_IN))->GetCheck()==BST_CHECKED)
		o_LogInByteStuffing1=true;
	else
		o_LogInByteStuffing1=false;
}
